const fs = require('fs');
const readline = require('readline');

const file = readline.createInterface({
  input: fs.createReadStream('input.txt'),
  output: process.stdout,
  terminal: false
});

let total = 0;
let allLines = '';

file.on('line', (line) => {
  allLines += line;
});

file.on('close', () => {

    // Get all indexes of instances of don't() in the string
    let dontIndexes = [];
    let i = -1;
    while ((i = allLines.indexOf("don't()", i+1)) !== -1){
      dontIndexes.push(i);
    }

    // Get all indexes of instances of do() in the string
    let doIndexes = [];
    i = -1;
    while ((i = allLines.indexOf("do()", i+1)) !== -1){
      doIndexes.push(i);
    }

    let cleanedLines = '';
    let shouldExclude = false;

    for(let i=0; i<allLines.length; i++) {
      if(dontIndexes.includes(i)) {
        shouldExclude = true;
      } else if(doIndexes.includes(i)) {
        shouldExclude = false;
      }
      if(!shouldExclude) {
        cleanedLines += allLines[i];
      }
    }

    // Use a regular expression to check for all instances of mul(number,number) in the string
    let matches = cleanedLines.match(/mul\((\d+),(\d+)\)/g);
    if(matches) {
      // Get the two numbers from each match
      let numbers = matches.map((match) => {
        return match.match(/mul\((\d+),(\d+)\)/).slice(1).map((num) => {return parseInt(num)});
      });
  
      // Multiply the two numbers and add to total
      numbers.forEach((nums) => {
        total += nums[0] * nums[1];
      });
    }

  console.log(total);
});